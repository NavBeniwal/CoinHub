...
public class RegisterPageValidationScript extends TestBase {
    ...
    @Test(priority = 42)
    public void verifyRegisterPageInvalidPhoneNumberErrorPopUpMsgFunctionality() throws IOException, InterruptedException {
        test=Reports.createTest(new Object() {}.getClass().getEnclosingMethod().getName(), reports);
        try {
            softAssert.assertEquals(registerPageValidation.validateRegisterPageInvalidPhoneNumberErrorPopUpMsg(PropertyReaderOptimized.getExcelSheetValue(30,1),test),true);
            test.log(LogStatus.PASS,"All conditions are verified.");
        } catch (Error | Exception e) {
            test.log(LogStatus.FAIL,"All conditions aren't verified.");
            Assert.fail(String.valueOf(e.getStackTrace()));
    ...
    @Test(priority = 43)
    public void verifyRegisterPageOtpRequiredValidationFunctionality() throws IOException, InterruptedException {
        test=Reports.createTest(new Object() {}.getClass().getEnclosingMethod().getName(), reports);
        try {
            softAssert.assertEquals(registerPageValidation.validateRegisterPageOtpRequiredValidation(PropertyReaderOptimized.getExcelSheetValue(26,1),test),true);
            test.log(LogStatus.PASS,"All conditions are verified.");
        } catch (Error | Exception e) {
            test.log(LogStatus.FAIL,"All conditions aren't verified.");
            Assert.fail(String.valueOf(e.getStackTrace()));
